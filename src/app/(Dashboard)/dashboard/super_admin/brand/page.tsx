...
import { TAbout, TBrand } from "@/types";
import { useDeleteMissionMutation, useGetAllMissionQuery } from "@/redux/api/missionApi";
import CreateBrandModal from "./_components/CreateBrandModal";
import UpdateBrandModal from "./_components/UpdateBrandModal";
...
const ServicePage = () => {
    ...
    const { data: brandsData, isLoading } = useGetAllBrandQuery({ page: currentPage, limit: 5 });
    const [deleteBrand] = useDeletlBrandMutation();
    ...
    const { meta, } = brandsData?.data || { meta: {}, oppresses: [] };
            ...
            <DashboardCard>
                <Box>
                    ...
                    <Box display='flex' justifyContent='space-between'>
                        <Typography variant="h5" fontWeight='bold'>Brand List </Typography>
                    ...
                    <Box bgcolor="white" padding={3}>
                        <TableContainer component={Paper}>
                            <Table aria-label="simple table">
                                ...
                                <TableBody>
                                    {brandsData?.data?.brands?.map((data: TBrand, index: number) => (
                                        <TableRow
                                            key={data._id}
