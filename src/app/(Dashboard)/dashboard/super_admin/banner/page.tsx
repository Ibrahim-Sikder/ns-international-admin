...
import CreateBannerModal from "./_components/CreateBannerModal";
import UpdateBannerModal from "./_components/UpdateBannerModal";
import { useDeletlBannerMutation, useGetAllBannerQuery } from "@/redux/api/bannerApi";
...
const LeaderPage = () => {
  ...
  const { data: bannerData, isLoading } = useGetAllBannerQuery({ page: currentPage, limit: 5 });
  const [deleteBanner] = useDeletlBannerMutation();
  ...
  const { meta, } = bannerData?.data || { meta: {}, oppresses: [] };
  ...
  return (
    <PageContainer title="NS-International " description="this is NS-International">
      <DashboardCard>
        <Box>

          <Box display='flex' justifyContent='space-between'>
            <Typography variant="h5" fontWeight='bold'>Banner List </Typography>
          ...
          <Box bgcolor="white" padding={3}>
            <TableContainer component={Paper}>
              <Table aria-label="simple table">
                ...
                <TableBody>
                  {bannerData?.data?.banners?.map((data: TBanner, index: number) => (
                    <TableRow
                      key={data._id}
                    ...
                    >
                      ...
                      <TableCell align="center">{data?.description?.slice(0, 20)}...</TableCell>
